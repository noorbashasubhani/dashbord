...
const Caluculation = ({ customerData, row_id , totals ,onUpdate}) => {
...
const Approve = async (cal_id, Type,partner_id,percentage) => {
  //alert(partner_id);
  ...
  if (Type === 'S') {
  bodyData.sup_status = 'S';
  bodyData.sup_approved_by = userId;
  bodyData.supper_partner_id = partner_id;
  bodyData.supper_partner_percentage = percentage;
} else if (Type === 'P') {
  bodyData.sales_status = 'S';              // ✅ Use 'partner_...'
  bodyData.partner_approved_by = userId;      // ✅ Fix this line
  bodyData.sales_partner_id = partner_id;
  bodyData.sales_partner_percentage = percentage;
} else if (Type === 'L') {
  bodyData.lead_status = 'S';
  bodyData.lead_approved_by = userId;
  bodyData.lead_partner_id = partner_id;
  bodyData.lead_partner_percentage = percentage;
}
        ...
        <table className="table table-stripped table-border table-sm table-bordered small">
            ...
            <tbody>
                <tr>
                    <td>Land Cost</td>
                    <td></td>
                    <td><input type="number" name="land_cost" className="form-control" value={form.land_cost || 0}
                ...
                <tr>
                    ...
                    <td><input type="number" name="loading_amount_on_land" value={form.loading_amount_on_land} 
                    className="form-control" onChange={handleChange} disabled/></td>
                </tr>
                <tr>
                 ...
                 <tr>
                    ...
                    <span
  className={`badge ${form.sup_status === 'A' ? 'bg-success' : 
    form.sup_status === 'S'  ? 'bg-warning' :  'bg-danger'}`}
  onClick={() => Approve(form._id, 'S',form.supper_partner_id,form.supper_partner_percentage)}
  style={{ cursor: 'pointer' }}
>
  {form.sup_status === 'S' ? 'Sent for Approval' : form.sup_status === 'P' ? 'Approve Here':'Approved'}
</span></td>
                 ...
                 <tr>
                    ...
                    <span
  className={`badge ${form.sales_status === 'A' ? 'bg-success' : 
    form.sales_status === 'S'  ? 'bg-warning' :  'bg-danger'}`}
  onClick={() => Approve(form._id, 'P',form.sales_partner_id,form.sales_partner_percentage)}
  style={{ cursor: 'pointer' }}
>
  
  {form.sales_status === 'S' ? 'Sent for Approval' : form.sup_status === 'P' ? 'Approve Here':'Approved'}
</span>
                    </td>
                ...
                <tr>
                    ...
                    <span
  className=
  {`badge ${form.lead_status === 'A' ? 'bg-success' : 
    form.lead_status === 'S'  ? 'bg-warning' :  'bg-danger'}`}
  onClick={() => Approve(form._id, 'L',form.lead_partner_id,form.lead_partner_percentage)}
  style={{ cursor: 'pointer' }}
>
  {form.lead_status === 'S' ? 'Sent for Approval' : form.lead_status === 'P' ? 'Approve Here':'Approved'}
</span>
                    </td>
               ...
               <tr className="table-primary">
                ...
                <tr className="bg-dark">
                    ...
                    <td></td>
                    <td><input type="number" name="cruise_cost" value={form.cruise_cost} className="form-control" onChange={handleChange} disabled/></td>
                </tr>
                <tr className="bg-dark">
  ...
  <div className="alert alert-danger p-2">
    <strong>Note:</strong> Total Partner Percentage must be ≤ <strong>12%</strong> and all partners must be <strong>Approved</strong> to save.
  </div>
)}
